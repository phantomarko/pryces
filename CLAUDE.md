...
- `interfaces.py` — all port ABCs: `StockProvider` (get_stocks), `StockRepository` (save_batch, get), `MessageSender` (send_message), `Logger` (debug, info, warning, error), `LoggerFactory` (get_logger(name) → Logger)
...
- `services.py` — `NotificationService` (sends stock notifications via `MessageSender`; injects `NotificationFormatter` and `clock: Callable[[], datetime]` (default `datetime.now`); `send_stock_notifications(stock)` calls `stock.generate_notifications(now=self._clock())` followed by `stock.drain_notifications(formatter)` and sends each message; delay window suppression is handled at the domain level by `Stock`), `StockSynchronizer` (fetches fresh stocks via `StockProvider`, merges with existing state from `StockRepository` via `Stock.update()`, syncs target prices via `Stock.sync_targets()`, persists via `save_batch`; `fetch_and_sync(symbols, targets) -> list[Stock]`, `persist(stocks)`)
