...
- `use_cases/get_stocks_statistics.py` — `GetStocksStatisticsRequest` (symbols: `list[str]`), `GetStocksStatistics` (injects `StockStatisticsProvider` and `StockStatisticsFormatter`; `handle(request) -> list[str]` — delegates to provider, formats each result via `StockStatistics.format(formatter)`, preserves symbol order)
...
- `report_stocks_statistics.py` — Standalone one-shot script (`ReportStocksStatisticsScript` class, argparse CLI). Entry point: `main()`. Args: `--debug`, `--verbose`. Reads all tracked symbols from `configs/` via `get_all_tracked_symbols()`, fetches and formats statistics via `GetStocksStatistics` (returns `list[str]`), sends all messages via `SendMessages`. Exits immediately after sending — designed for scheduled (cron) use. Composition root (`_create_script`) wires `YahooFinanceStatisticsProvider`, `GetStocksStatistics` (with `RegularStockStatisticsFormatter`), `TelegramMessageSender`, and `SendMessages`.
