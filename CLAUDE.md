...
- `stocks.py` — `MarketState` enum (OPEN, PRE, POST, CLOSED), `InstrumentType` enum (STOCK, ETF, CRYPTO, INDEX), `StockSnapshot` frozen dataclass (captures mutable price/market fields), `Stock` entity (symbol, current_price + optional: name, currency, market_state, kind, and 10 Decimal price fields; owns `_targets: list[TargetPrice]` and `_fulfilled_targets: list[TargetPrice]` internally; `update(source)` captures current state as `StockSnapshot` then copies fields from source, preserves accumulated notifications and targets; `is_market_state_transition()` checks snapshot vs current market state; `sync_targets(target_values: list[Decimal])` creates `TargetPrice` internally, matches incoming values vs existing by `.target` — keeps existing (preserves entry price), creates new ones (calls `set_entry_price(self)`), removes targets no longer in incoming list; `generate_notifications() -> None` parameterless, generates notifications into `_pending_notifications` staging list with domain-level deduplication by `NotificationType` across both `_notifications` (historical) and `_pending_notifications` — `TARGET_PRICE_REACHED` is never deduplicated; removes triggered targets from `_targets` and accumulates them in `_fulfilled_targets`; market open/close notifications are skipped entirely for `InstrumentType.CRYPTO` (crypto markets run 24/7); when `REGULAR_MARKET_OPEN` is first generated, all other notifications are deferred to the next call (avoids notification burst on market open); target notifications only generated when market is OPEN; generation order within a market-open cycle matters: SMA50 crossed → close-to-SMA50 → SMA200 crossed → close-to-SMA200 → 52-week high/low → percentage change → session gains/losses erased → target prices; close-to-SMA notifications are suppressed when the corresponding SMA crossing has already been notified (checks both historical and pending); percentage change thresholds are instrument-type-aware: STOCK uses 5/10/15/20% (`_STOCK_INCREASE_THRESHOLDS`, `_STOCK_DECREASE_THRESHOLDS`), all other types including None use 2.5/5/7.5/10% (`_DEFAULT_INCREASE_THRESHOLDS`, `_DEFAULT_DECREASE_THRESHOLDS`); each threshold maps to a level-based `NotificationType` (LEVEL_1..4); percentage change notifications are silently moved to historical `_notifications` (suppressed from current cycle) when any SMA notification (crossed or close-to) or 52-week notification (high or low) is pending — avoids redundant messages when those events already convey the price move; 52-week high/low notifications use the common price-change prefix format with percentage change, require `previous_close_price` to generate; `drain_notifications() -> list[str]` moves pending notifications into the historical `_notifications` list (for future dedup) and returns their messages — follows generate → drain → historical lifecycle; `drain_fulfilled_targets() -> list[Decimal]` returns target values from fulfilled targets and clears the accumulator)
- `notifications.py` — `NotificationType` enum (CLOSE_TO_SMA50, CLOSE_TO_SMA200, SMA50_CROSSED, SMA200_CROSSED, REGULAR_MARKET_OPEN, REGULAR_MARKET_CLOSED, LEVEL_1_INCREASE, LEVEL_2_INCREASE, LEVEL_3_INCREASE, LEVEL_4_INCREASE, LEVEL_1_DECREASE, LEVEL_2_DECREASE, LEVEL_3_DECREASE, LEVEL_4_DECREASE, SESSION_GAINS_ERASED, SESSION_LOSSES_ERASED, NEW_52_WEEK_HIGH, NEW_52_WEEK_LOW, TARGET_PRICE_REACHED), `Notification` class (factory-based construction with static creators; `create_percentage_change(notification_type, symbol, current_price, change_percentage)` single factory for all percentage level notifications)
...
- `bot_commands.py` — `BotCommand` ABC (name, usage, description, arg_count, execute), `TargetsCommand` (lists target prices for a symbol), `TargetAddCommand` (adds a target price to a symbol's config), `TargetRemoveCommand` (removes a target price from a symbol's config), `SymbolAddCommand` (adds a new symbol entry to a named config; injects `_FindConfigByNameFn`), `SymbolRemoveCommand` (removes a symbol from whichever config contains it; guards against removing the last symbol; injects `_FindConfigFn`), `SymbolsCommand` (lists all tracked symbols across configs; injects `_GetAllSymbolsFn`), `ConfigsCommand` (lists all config file names without path or extension; injects `_GetConfigNamesFn`), `HelpCommand` (formats usage for all commands); `_FindConfigFn`, `_FindConfigByNameFn`, `_GetAllSymbolsFn`, `_GetConfigNamesFn` type aliases; `_find_symbol_config`, `_update_symbol_prices`, `_add_symbol_to_config`, `_remove_symbol_from_config` private helpers shared by command implementations; `BotCommandDispatcher` (routes text to commands by name, returns empty string for non-commands, error message for unknown commands or wrong arg count; injects `logger_factory: LoggerFactory`, logs command name and args on execution)
