...
- `interfaces.py` — `StockProvider` ABC (port), `MessageSender` ABC (port), `NotificationRepository` ABC (port: `save`, `exists_by_type`), `MarketTransitionRepository` ABC (port: `save`, `get`, `delete` — tracks first-detected market state transition timestamp per symbol), `TargetPriceRepository` ABC (port: `get_all`, `save`, `delete` — stores `PriceTarget` entities keyed by symbol+target_price)
- `dtos.py` — `StockDTO` (maps domain Stock to DTO, includes marketState), `TargetPriceDTO` (symbol + target Decimal; `to_price_target()` converts to domain `PriceTarget`)
