...
- `providers.py` — `YahooFinanceSettings` frozen dataclass (max_workers, extra_delay_in_minutes), `YahooFinanceMapper` (converts raw yfinance info dicts to domain `Stock` objects — maps MarketState from yfinance values, handles price fallback chain, adds `extra_delay_in_minutes` to yfinance-reported delay when delay > 0), `YahooFinanceProvider` implements `StockProvider` via `yfinance` (delegates mapping to `YahooFinanceMapper`)
