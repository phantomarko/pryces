...
- `report_stocks_statistics.py` — Standalone one-shot script (`ReportStocksStatisticsScript` class, argparse CLI). Entry point: `main()`. Args: `--debug`, `--verbose`. Reads all tracked symbols from `configs/` via `get_all_tracked_symbols()`, delegates the full fetch → format → send pipeline to `TriggerStocksStatistics`. Exits immediately — designed for scheduled (cron) use. Composition root (`_create_script`) wires `YahooFinanceStatisticsProvider`, `TelegramMessageSender`, and `TriggerStocksStatistics` (with `RegularStockStatisticsFormatter`).
