...
- `bot_commands.py` — `BotCommand` ABC (name, usage, description, arg_count, execute), `TargetsCommand` (lists target prices for a symbol), `TargetAddCommand` (adds a target price to a symbol's config), `TargetRemoveCommand` (removes a target price from a symbol's config), `HelpCommand` (formats usage for all commands); `BotCommandDispatcher` (routes text to commands by name, returns empty string for non-commands, error message for unknown commands or wrong arg count)
