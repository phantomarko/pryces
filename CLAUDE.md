...
- `configs.py` — `CONFIGS_DIR: Path` (project root `configs/` directory); `SymbolConfig` frozen dataclass (symbol, prices as `list[Decimal]` — no validation), `MonitorStocksConfig` frozen dataclass (interval, symbols as `list[SymbolConfig]` — with validation), `ConfigManager` (loads/writes JSON config file, raises `ConfigLoadingFailed` on any read error; per-symbol mutators `replace_symbol_prices(symbol, prices)`, `add_symbol(symbol)`, `remove_symbol(symbol)` re-read the config on disk, rebuild the dataclass, and write it back); `find_config_for_symbol(symbol) -> tuple[Path, MonitorStocksConfig] | None` (scans `CONFIGS_DIR` alphabetically, returns first config containing the symbol); `find_config_by_name(config_name) -> tuple[Path, MonitorStocksConfig] | None` (looks up config by name, returns None if not found or load fails); `get_all_tracked_symbols() -> list[str]` (scans all configs, collects unique symbols, returns sorted list); `get_all_tracked_symbols_with_targets() -> list[tuple[str, list[Decimal]]]` (scans all configs sorted alphabetically, collects unique symbols with their target prices — first-seen config wins for deduplication, returns sorted by symbol); `get_config_names() -> list[str]` (returns sorted list of config file stems from `CONFIGS_DIR`, no file content read) — single source of truth for config-file persistence used by both the console commands and the Telegram bot
