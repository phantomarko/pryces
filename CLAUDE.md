...
- `stocks.py` — `MarketState` enum (OPEN, PRE, POST, CLOSED), `StockSnapshot` frozen dataclass (captures mutable price/market fields), `GenerateNotificationsResult` frozen dataclass (`new_notifications: list[Notification]`), `Stock` entity (symbol, current_price + optional: name, currency, market_state, and 10 Decimal price fields; owns `_targets: list[TargetPrice]` internally; `update(source)` captures current state as `StockSnapshot` then copies fields from source, preserves accumulated notifications and targets; `is_market_state_transition()` checks snapshot vs current market state; `sync_targets(targets)` matches incoming vs existing by `.target` value — keeps existing (preserves entry price), adds new ones (calls `set_entry_price(self)`), removes targets no longer in incoming list; `generate_notifications()` parameterless, generates notifications with domain-level deduplication by `NotificationType` on accumulated `_notifications` list — `TARGET_PRICE_REACHED` is never deduplicated; removes triggered targets from `_targets`; returns `GenerateNotificationsResult`; target notifications only generated when market is OPEN)
...
- `monitor_stocks.py` — Standalone monitor script (`MonitorStocksScript` class, argparse CLI, logging). Entry point: `main()`. Passes target prices from config into `TriggerStocksNotificationsRequest.targets` on each iteration. `_write_config(fulfilled)` removes fulfilled `TargetPriceDTO` prices from `self._config` symbols (keeping the symbol entry even when all its prices are removed), then persists the updated config to disk via `ConfigManager.write_monitor_stocks_config` if anything changed. Composition root wraps `TelegramMessageSender` in `FireAndForgetMessageSender` for non-blocking sends; `_ScriptContext` groups the script and sender so `main()` calls `shutdown()` in a `finally` block.
