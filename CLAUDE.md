...
- `stock_statistics.py` — `StatisticsPeriod` enum (1D, 1W, 3M, 1Y, YTD), `HistoricalClose` frozen dataclass (period, close_price), `PriceChange` class (period, close_price, current_price → computes `change` and `change_percentage` via `calculate_percentage_change()` in `__init__`; all fields exposed as read-only properties), `StockStatistics` entity (symbol, current_price, historical_closes, name: `str | None`, currency: `str | None`; converts `list[HistoricalClose]` to `list[PriceChange]` on construction; `price_changes` returns immutable list)
...
- `dtos.py` — `StockDTO` (maps domain Stock to DTO, includes market_state, kind, cap_size, and currency: all `str | None` — `currency` is converted from `Currency` enum via `.value` in `from_stock()`), `TargetPriceDTO` (symbol + target Decimal), `PriceChangeDTO` (period, close_price, change, change_percentage — mapped from domain `PriceChange` via `from_price_change()`), `StockStatisticsDTO` (symbol, current_price, price_changes: `list[PriceChangeDTO]`, name: `str | None`, currency: `str | None` — mapped from domain `StockStatistics` via `from_stock_statistics()`)
...
- `use_cases/get_stocks_statistics.py` — `GetStocksStatisticsRequest` (symbols: `list[str]`), `GetStocksStatistics` (injects `StockStatisticsProvider`; `handle(request) -> list[StockStatisticsDTO]` — delegates to provider, maps results to DTOs, preserves symbol order)
...
- `commands/monitor_stocks.py` — `MonitorStocksCommand` (dynamic config picker from `CONFIGS_DIR` via `get_config_files()`; prompts for duration and extra delay; launches the standalone monitor script as a detached background process via `subprocess.Popen`, returns PID; returns `success=False` on `OSError` from `Popen`; returns error if no configs found)
...
- `report_stocks_statistics.py` — Standalone one-shot script (`ReportStocksStatisticsScript` class, argparse CLI). Entry point: `main()`. Args: `--debug`, `--verbose`. Reads all tracked symbols from `configs/` via `get_all_tracked_symbols()`, fetches price statistics via `GetStocksStatistics`, formats each result via `format_stats` from `formatters`, sends all messages via `SendMessages`. Exits immediately after sending — designed for scheduled (cron) use. Composition root (`_create_script`) wires `YahooFinanceStatisticsProvider`, `GetStocksStatistics`, `TelegramMessageSender`, and `SendMessages`.
