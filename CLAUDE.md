...
- `stocks.py` — `MarketState` enum (OPEN, PRE, POST, CLOSED), `StockSnapshot` frozen dataclass (captures mutable price/market fields), `Stock` entity (symbol, current_price + optional: name, currency, market_state, and 10 Decimal price fields; `update(source)` captures current state as `StockSnapshot` then copies fields from source; `is_market_state_transition()` checks snapshot vs current market state; `generate_notifications(targets)` accepts `list[TargetPrice]`, generates all notifications including target price reached, returns `list[TargetPrice]` of triggered targets; target notifications only generated when market is OPEN)
...
- `services.py` — `NotificationService` (sends stock milestone notifications via MessageSender; delegates deduplication to injected `NotificationRepository`; `send_stock_notifications(stock, targets)` calls `stock.generate_notifications(targets)` which generates all notifications including target price reached, then sends all unseen notifications in a single loop — skips dedup for `TARGET_PRICE_REACHED` type (returns fulfilled `list[TargetPrice]`); suppresses all notifications (including targets) when `stock.price_delay_in_minutes > 0` and `stock.is_market_state_transition()` detects a transition to OPEN/POST — delay window tracked via injected `MarketTransitionRepository`; accepts injectable `clock: Callable[[], datetime]` for testing)
